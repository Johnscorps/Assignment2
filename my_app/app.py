...
def images_remove_all():
    ...
    """
    output=docker('rmi',docker('images','-a','-q'))
    resp = output
    return Response(response=resp, mimetype="application/json")
...
def images_create():
    ...
    return Response(response='{"image": "%s"}'% dockerfile, mimetype="application/json")
...
def containers_update(id):
	...
	curl -X PATCH -H 'Content-Type: application/json' http://localhost:8080/containers/b6cd8ea512c8 -d '{"state": "$
    curl -X PATCH -H 'Content-Type: application/json' http://localhost:8080/containers/b6cd8ea512c8 -d '{"state": "$
    """
    body = request.get_json(force=True)
    try:
...
def images_update(id):
	...
	curl -s -X PATCH -H 'Content-Type: application/json' http://localhost:8080/images/7f2619ed1768 -d '{"tag": "tes$

    """
    
    output=docker('update',id)
    resp = output
    return Response(response=resp, mimetype="application/json")
